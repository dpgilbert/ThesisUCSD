...
Taken from \cite{MT2_2019}.}
  ...
  Finally, signal models must be simulated in the fast-simulation framework discussed in Section~\ref{sec:pipeline} to make the enormous array of signal scenarios considered computationally feasible.
...
\section{Disappearing Tracks Search}
  ...
  The disappearing tracks produced by \chargino decays tend to be very high quality and highly isolated, with negligible energy deposits in any of the calorimeter cells along the track's extrapolated path.
  ...
  Alongside isolation, these quality metrics allow for signal-like disappearing tracks to be distinguished from disappearing tracks that are very likely to be due to a reconstruction error.
  ...
  With no disappearing track, event fail the ST selection, so signal efficiencies for long \chargino lifetime are reduced in the large mass splitting scenario, and limits are weakened.
