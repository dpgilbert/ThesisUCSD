...
\begin{table}
\centering
\topcaption[Comparison of theoretical prediction and experimental measurement of the electron magnetic moment.]
...
Modern particle physicists hope to find experimental clues by testing the Standard Model using any available means, including, as in this work, by comparing its predictions for the outcomes of high energy proton collisions with experimental data.
  ...
  \begin{table}
    \centering
    \topcaption[Table of Standard Model bosons.]
  ...
  \begin{table}
    \centering
    \topcaption[Table of Standard Model fermions.]
...
~
  ...
  The only (observationally) stable hadron is the proton, the lightest baryon, excepting neutrons bound to protons by the QCD equivalent of dipole forces in atomic nuclei.
  ...
  \begin{figure}[h!]
    ...
    \caption[The QCD quark-quark potential, calculated numerically using lattice techniques.]{
  ...
  Non-perturbative approaches to QCD exist, including the lattice numerical method used to produce the plot shown in Figure~\ref{fig:QCDpotential}, but they are extremely computationally expensive for even the most simple systems.
  ...
  \begin{figure}[h!]
    ...
    \caption[Proton parton distribution function.]{
      ...
      Since the heavy quark and antiquark curves overlap, only the quark curves are shown and represent both.
...
This section introduces a few of the known problems with the Standard Model.
  ...
  \end{equation}
  where $\mu$ is a free parameter called the bare mass of the Higgs, and $\Delta m_h$ is an adjustment that the Higgs acquires via its interactions with other fields in the Standard Model,
  ...
  A model that produces an output of a very different order of magnitude from the inputs is said to be finely tuned.
  ...
  By far the most popular proposal for such a replacement is a supersymmetric extension to the Standard Model, discussed in Section~\ref{sec:SUSY}.
...
\approx
  ...
  No modification of gravity has ever been found that can satisfactorily explain both the rotation curve observations, and these energy density measurements.
  ...
  No excess of such events is observed in microlensing and complementary searches, so that if all dark matter has exactly the same mass, it cannot have any mass in the range $10^{-7}~\mathrm{M}_{\mathrm{Sun}} < \mathrm{M}_{\mathrm{DM}} < 10^5~\mathrm{M}_{\mathrm{Sun}}$ \cite{primordialBH_dist}.
  ...
  The characteristic energy scale of the weak interaction is $\sim$100~GeV.
  ...
  The chief argument against WIMPs is that they have not yet been observed in experiments despite significant efforts, with some of the more stringent general constraints produced by the XENON1T experiment \cite{xenon1t}, yet some of the parameter space remains viable.
  It is a high priority of current experiments to extend sensitivity into these regions, and either discover or exclude WIMP dark matter.
